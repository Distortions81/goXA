...
func showUsage() {
	...
	fmt.Println("  -span N         split archive when file exceeds N bytes (0=disable)")
	...
	fmt.Println("  -fec-level L    FEC redundancy preset: low (~30%), medium (~50% similar to optical media), high (~100%)")
...
func initFlags() (*flag.FlagSet, *flagSettings) {
	...
	fs.Int64Var(&spanSize, "span", defaultSpanSize, "span file size limit in bytes (0=disable)")
	...
	fs.StringVar(&f.fecLevel, "fec-level", "", "FEC redundancy preset: low(~30%)|medium(~50% similar to optical media)|high(~100%)")
